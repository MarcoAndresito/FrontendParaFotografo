...
const Album = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:7062/api/albumes/${id}/fotos`)
      .then((res) => res.json())
      .then((data) => {
        setPhotos(data);
        setLoading(false);
        ...
        {photos.map((photo) => (
          ...
          >
            <img
              ...
              alt={photo.nombreArchivo}
              className={styles.foto}
            />
            <p>{photo.nombreArchivo}</p>
          </div>
        ))}
      </Slider>
        ...
        <div className={styles.modal} onClick={() => setSelectedPhoto(null)}>
          ...
          >
            <img
              ...
              alt={selectedPhoto.nombreArchivo}
              className={styles.foto}
            />
            <p>{selectedPhoto.nombreArchivo}</p>
            <p>Formato: {selectedPhoto.formato}</p>
            <p>
              Resolución: {selectedPhoto.anchoPx}x{selectedPhoto.altoPx}
            </p>
            <p>Peso: {selectedPhoto.pesoKB} KB</p>
